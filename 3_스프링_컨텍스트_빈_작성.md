...
public class ProjectConfig{
    ...
    @Bean
    public Person person(Parrot parrot){ // 스프링은 이 매개변수에 앵무새 빈을 주입
        Person p = new Person();
        p.setName("Ella");
        p.setParrot(parrot);
        return p;
...
스프링에서 매개변수 또는 클래스 필드에 값을 주입할 때 동일한 타입의 빈이 여러 개 있을 때
...
매개변수에 의존하는 방식은 다른 개발자가 실수로 쉽게 리팩터링하고 변경할 수 있으므로 피하는 것이 실제 프로젝트에서 일반적이다.
```java
@Configuration
public class ProjectConfig {
...
대신 @Qualifier 애너테이션을 사용하여 애너테이션의 값 속성으로 주입하려는 빈을 지정할 수 있다.
```java
@Configuration
public class ProjectConfig{
        @Bean
...
@Autowired 애너테이션을 사용할 때도 메서드 이름으로 정한 컨텍스트 빈 이름을 이용하거나 @Qualifier를 사용하여 동일한 타입의 빈들 중 하나를 선택할 수 있다.
